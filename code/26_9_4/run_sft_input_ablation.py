...
``direct_no_history_sft``
    Query -> Gold。训练和测试均不输入历史，用于隔离纯 Task Adaptation 收益。
``direct_top1_sft``
    Query + BM25 Top-1 History -> Gold。训练和测试严格保持 k=1。
``editor_sft``
...
def direct_history_k(experiment: str, configured_maximum: int) -> int:
    """Return the matched train/test history count for direct-generation rows."""

    if experiment in {"base_query_only", "direct_no_history_sft"}:
        return 0
    if experiment in {"base_top1", "direct_top1_sft"}:
        return 1
    if experiment in {"base", "direct_sft"}:
        return configured_maximum
    raise ValueError(f"experiment={experiment} 不是 direct-generation 协议")
...
def compile_examples(
    ...
    for row in rows:
        ...
        if experiment in {
            ...
            history_k = direct_history_k(experiment, maximum_history)
            prompt = base_prompt(
                row,
                history_k,
                include_history=history_k > 0,
            )
            parent = None
        else:
...
def main() -> None:
    ...
    if args.experiment in {"base", "base_top1", "base_query_only"}:
        if args.stage not in {"eval", "all"}:
            raise ValueError("base 类实验只支持 --stage eval/all")
    else:
    ...
    if args.stage in {"eval", "all"}:
        ...
        adapter = None
        if args.experiment not in {"base", "base_top1", "base_query_only"}:
