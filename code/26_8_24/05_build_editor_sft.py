...
S0 直接从四个 target-blind Parent 构造 ``prompt -> {output: Gold}``；
Gold-aware 模式保留为后续对照。两种模式的 Student Prompt 都看不到 Gold。
"""
...
def _output_only_example(
    row: dict[str, Any], parent: dict[str, Any], config: dict
) -> dict[str, Any]:
    ...
    return {
        ...
        "output_text": json.dumps(
            {"output": str(row["target"])}, ensure_ascii=False, separators=(",", ":")
        ),
    }
...
def build(config: dict) -> dict[str, Any]:
    ...
    for row in rows if mode not in conditional_modes else []:
        ...
        for value in values:
            if mode == "output_only" or (mode == "atomic_trace" and "parent_id" not in value):
                example = _output_only_example(row, value, config)
