...
def extract_json(
    ...
    plain_output_only: bool = False,
    base_text_protocol: bool = False,
) -> dict[str, Any]:
    """解析 Editor 响应。

    output-only 消融的监督目标只有一个标题。Llama 偶尔会把对象结尾写成
    ``{"output":"..." ]``，这不是标题错误，而是包装 JSON 的局部语法错误。
    这里先尝试严格 JSON；失败后只在明确找到 ``output`` 字段时恢复其字符串值，
    不再把整个损坏 JSON 当作标题。没有可恢复字段时仍返回错误，保持失败样本
    进入统一分母，避免因宽松回退造成评估偏差。
    """
    ...
    if base_text_protocol:
        # 与 Base 的标准评估完全一致：取第一行非空标题，去除常见前缀，
        # 不把换行或长度上限当成模型失败。格式违规另由诊断字段记录。
        value = re.sub(r"^(?:title|output)\s*:\s*", "", value, flags=re.I).strip()
        value = next((line.strip().strip('"') for line in value.splitlines() if line.strip()), "")
        if not value or value.startswith("{") or value.startswith("["):
            raise ValueError("base-text protocol 未生成可用标题")
        return {"output": value}
    if plain_output_only:
        # 真正的纯文本消融拒绝 JSON、Markdown 和多行解释；返回 dict 只是
        # 为了复用候选池内部契约，训练/rollout 的 response 仍是纯标题。
        if not value or value[:1] in {"{", "["} or "\n" in value or len(value) > 300:
            raise ValueError("plain output 必须是一行不超过 300 字符的标题")
        return {"output": value}
    candidates = [value]
    ...
    for item in candidates:
        try:
            ...
            if output_only and not isinstance(parsed.get("output"), str):
                raise ValueError("output-only 响应缺少字符串 output 字段")
            return parsed
        except (json.JSONDecodeError, ValueError) as current:
            error = current
    if output_only:
        # 轻微损坏的 JSON：定位 output 字段并解析其字符串，不吞掉前缀或
        # ``{"output":...}`` 包装。支持错误的 ]/} 结尾和缺失结尾引号。
        field = re.search(r"[\"']output[\"']\s*:\s*", value, re.I)
        if field:
            rest = value[field.end() :].lstrip()
            if rest[:1] in {'"', "'"}:
                quote = rest[0]
                chars: list[str] = []
                escaped = False
                ...
                raw = "".join(chars)
                # 让 JSON 解码器处理常见的 \\n、\\" 转义；单引号包装则保留
                # 内容本身，避免把标题中的撇号误解释为结构符号。
                if quote == '"':
                    try:
                        raw = json.loads('"' + rest[1 : closing or len(rest)] + '"')
                    except json.JSONDecodeError:
                        pass
            else:
                raw = re.split(r"[}\]]", rest, maxsplit=1)[0].strip().strip(",")
        else:
            # 纯文本是模型没有输出包装的情况；只接受单行标题，不接受任意
            # 多行解释，避免把自由文本 rationale 当成标题。
...
class LocalEditor:
    ...
    def __init__(self, config: dict):
        ...
        base_only = bool(config.get("model", {}).get("base_only", False))
        ...
        if not base_only and not adapter_path.exists():
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_path if base_only else adapter_path,
            use_fast=True,
        ...
        self.plain_output_only = str(
            config.get("sft_data", {}).get("supervision_mode", "")
        ) == "plain_output_only"
        self.base_text_protocol = str(
            config.get("sft_data", {}).get("prompt_protocol", "")
        ) == "base_text"
        ...
        for start in range(0, len(prompts), batch_size):
            ...
            try:
                ...
                for raw_value in raw_values:
                    raw = raw_value.strip()
                    try:
                        results.append((extract_json(raw, self.output_only, self.plain_output_only, self.base_text_protocol), raw, None))
                ...
                # 长 Prompt 的 KV cache 和临时 logits 即使不再被引用，也可能
                # 留在 PyTorch caching allocator 中；rollout 会循环数千个 Query，
                # 每个小批结束后释放缓存可避免碎片累积导致后续搬运 OOM。
    ...
    for parent, (payload, raw, generation_error) in zip(
        ...
        try:
            ...
            if supervision_mode in {
                "output_only", "plain_output_only", "atomic_trace", "conditional_preference_trace",
                "simple_conditional_trace",
            }:
